...
public class MainActivity extends Activity implements OnItemClickListener ,OnClickListener{
	...
	protected static String selectedMessage=null;//所点击的项
	...
	private String mainService=null;//表示目前对话的上下文是在哪一项主服务中
	private String branchService=null;//表示目前对话的上下文是在主服务中的哪一项分支服务中
				...
				switch(service){
					...
					case "message":{//2 短信相关服务
						...
						switch(operation){
							...
							case "SENDCONTACTS":{//3发送名片,目前只能识别：名字发给名字
  ...
  //开始语义分析
    ...
    private void printResult(RecognizerResult results,boolean isLast) {
		...
		SRResult=resultBuffer.toString();//反馈结果至SRResult
		if(isLast==true){
			speak(SRResult, true);
			if(SRResult.equals("查看数据库。")){//特殊指令，查看数据库，不进行服务分析
				QueryDB queryDB=new QueryDB(this);
				queryDB.start();
			}
			else{
				startAnalysis();//转入服务分析
			}
			//加入数据库
	...
	@Override
	public void onItemClick(AdapterView<?> parent, View view, int pos, long id) {//点击对话框
	...
	public void onCreateContextMenu(ContextMenu menu, View v,ContextMenu.ContextMenuInfo menuInfo) {//长按对话框
		//speak("hai",false);
		super.onCreateContextMenu(menu, v, menuInfo);
        //获取长按项的值
		int pos=((AdapterView.AdapterContextMenuInfo) menuInfo).position;
		SiriListItem item = list.get(pos);
		selectedMessage=item.message;//获取所点击项的内容 
		//speak(item.message,false);
		menu.add(0, LIKE, 0, "这个回答好赞");
		menu.add(0, DISLIKE, 0, "这个回答好水");
	}
